...
(function($, window, document, undefined) {
	...
	WindowObj.prototype = {
		...
		verticalMove: function() {
			...
			function markStage() {
				...
				if (stageIndex != 0) {
					...
					$(controlList).removeClass('select');
					$(controlList).eq(stageIndex).addClass('select');
				} else {
					$(controlList).removeClass('select');
				}
			}
			...
			function addSpecial() {
				...
				var guiderID;
				$(controlList).not(".inner").click(function() {
					if ($(this).index() > 1) {
		...
		horizontalMove: function() {
			...
			function addSpecial() {
				/* 起始页面为2015 */
				$(controlList).eq(7).click(); //不先加这句话就是给自己挖坑
			}
			setTimeout(function() {
				addSpecial();
			}, 200); //bug fix:不加setTimeout最后一层没执行指令
			...
			function markIndex() {
				...
				if (obj.prevObject[0] === $('#stage2').children('.pageBox').prevObject[0]) {//为了仅执行一次
					...
					if (($('.pageSelect').index() * 166 - $('.year').scrollLeft()) > 664) {
						$('.showright').click();
					}
				}
			}
			...
			function goLeft() {
				...
				// $(controlList).eq(pageIndex-1).click();
			}
